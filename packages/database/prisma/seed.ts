...
async function main() {
  ...
  const skills = [
    ...
    },
    {
      ...
      instructions: "To prepare for a meeting:\n1) call list_upcoming_meetings, then identify the specific meeting(s) the user means (e.g. 'tomorrow' = meetings on that date; 'the Kindro DSU' = that title). If the user is vague and there are several, prepare the 1-3 most relevant and say which you chose.\n2) For EACH chosen meeting, call search_project_knowledge with SPECIFIC queries built from the meeting's title and distinctive attendee names/topics (e.g. 'Kindro Care DSU action items', 'Wave CRM integration decisions') — NEVER search the literal word 'meeting'. Run more than one query if useful.\n3) Ground the brief ONLY in what the searches return: recap of the last related discussion, decisions made, and open action items, each tied to its source. If the knowledge base has nothing relevant for a meeting, say 'No prior context found in this project' — do NOT invent generic 'focus' text.\n4) Output per meeting: when & attendees, what happened last time (grounded), open items to follow up, and 3-5 concrete talking points.\nNote: the knowledge base is per-project, so you can only ground meetings that belong to this project.",
