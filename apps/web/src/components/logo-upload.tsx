...
export function LogoUpload({
  ...
  const [preview, setPreview] = useState<string | null>(
    companyId && existing
      ? `${API_BASE}/companies/${companyId}/logo?v=${Date.now()}`
      : null,
  );
  ...
  const clear = async () => {
    ...
    // Immediate mode (edit): always delete from the server — covers both a
    // pre-existing logo and one just uploaded in this same session.
    if (companyId) {
      setBusy(true);
      try {
    ...
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <p className="text-xs text-muted mt-1">
          PNG, JPG, WebP or SVG · optional · auto-cropped to a square
        </p>
...
export function CompanyLogo({
  ...
  if (!hasLogo) {
    return (
      <div
        className="grid place-items-center rounded-lg bg-surface-2 text-muted shrink-0"
