    ...
    <section className="rounded-lg border border-blue-900/40 bg-gray-900 p-4">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {SUGGESTIONS.map((s) => (
            <button key={s} onClick={() => { setQuestion(s); ask(s); }}
              className="rounded border border-gray-700 bg-gray-800 px-2 py-1 text-xs text-gray-400 hover:text-gray-200 hover:border-gray-600 transition-colors">
              {s}
            </button>
          ))}
        </div>
      ...
      {result && (
        <div className="rounded border border-gray-700 bg-gray-800/50 p-3 space-y-2">
          <p className="text-sm text-gray-100 whitespace-pre-wrap">{result.answer}</p>
          <div className="flex items-center gap-3 pt-1 border-t border-gray-700">
            <span className="text-xs text-gray-500">{result.sources} source{result.sources !== 1 ? "s" : ""}</span>
            <span className="text-xs text-gray-600">·</span>
            <span className={`text-xs ${result.usedVectorSearch ? "text-blue-400" : "text-gray-500"}`}>
              {result.usedVectorSearch ? "vector search" : "text search"}
            </span>
            <button onClick={() => setResult(null)} className="ml-auto text-xs text-gray-600 hover:text-gray-400">Clear</button>
          </div>
        </div>
      )}
    </section>
