...
export default async function StandaloneDiagramEditorPage({ params }: { params: Promise<{ diagramId: string }> }) {
  const { diagramId } = await params;
  const diagram = await serverFetch<DiagramRecord>(`/diagrams/${diagramId}`, null);

  if (!diagram) {
    return (
      <Shell active="/diagrams" width="full">
        <Link href="/diagrams" className="inline-flex items-center gap-1 text-xs text-muted hover:text-foreground mb-4">
  ...
  return (
    <Shell active="/diagrams" width="full">
      <Link href="/diagrams" className="inline-flex items-center gap-1 text-xs text-muted hover:text-foreground mb-3">
      ...
      {isMermaidKind(diagram.kind) ? (
        ...
        <DiagramCanvas projectId="" diagramId={diagram.id} initial={initial} />
      )}
    </Shell>
  );
}
